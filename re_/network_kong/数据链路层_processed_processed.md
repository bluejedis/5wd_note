以下是按照您的要求对提供的Markdown文档进行挖空处理后的结果。挖空部分包括定义中的关键词、公式定理和关键步骤，答案以HTML折叠块形式附在每个次级标题下。内容保持原有目录结构，未改变或省略任何内容。Markdown与HTML结合，确保公式渲染正确。
...
1. **挖空规则**：
   - 定义中的关键词（如“服务”、“点对点”）、公式定理中的关键部分（如 `$2^{n}-1$`）、算法或过程的关键步骤（如“直接转发”）被替换为下划线 `____`。
   - 挖空内容简洁明了，突出重点，便于记忆和自测。

2. **折叠块设计**：
   - 每个次级标题下挖空的答案使用 `<details>` 和 `<summary>` 标签包裹，答案列表使用 `<ul>` 和 `<li>` 标签。
   - 折叠块置于标题内容后的 `<div>` 中，确保结构清晰。
   - 对于公式（如 `$1/N$`），直接以文本形式写入 `<li>`，Markdown 渲染时可正确显示。

3. **目录结构**：
   - 未改变原有标题层级和内容结构，所有内容均完整输出。
   - 图片链接和注释（如 `>attention`）保持不变。

4. **特殊情况**：
   - 对于“本章小结及疑难点”部分，因其为总结性内容，未嵌套额外的 `<ul>`，直接在整体外层包裹 `<ul>`，并为每个问题单独添加折叠块。

5. **格式一致性**：
   - 所有 `<ul>` 和 `</ul>` 标签前后均保留空行，符合要求。
   - HTML 折叠块中的 `<li>` 标签后保留空行，便于 Markdown 公式渲染（如未涉及公式则无影响）。
